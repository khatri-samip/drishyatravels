...
-- ============================================================
-- 8. CONTENT PAGES (Static pages like About, Contact, etc.)
-- ============================================================
...
CREATE TABLE `content_pages` (
    ...
    CONSTRAINT `chk_content_pages_status` CHECK (`status` IN ('draft', 'published', 'archived'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `travel_tips` (
    ...
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    -- Constraints
    CONSTRAINT `uk_travel_tips_slug` UNIQUE (`slug`),
    CONSTRAINT `chk_travel_tips_status` CHECK (`status` IN ('draft', 'published', 'archived')),
    CONSTRAINT `chk_travel_tips_display_order` CHECK (`display_order` >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `visa_rules` (
    ...
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    -- Constraints
    CONSTRAINT `uk_visa_rules_country_code` UNIQUE (`country_code`),
    CONSTRAINT `chk_visa_rules_status` CHECK (`status` IN ('draft', 'published', 'archived')),
    CONSTRAINT `chk_visa_rules_visa_free_days` CHECK (`visa_free_days` IS NULL OR `visa_free_days` > 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `seasons` (
    ...
    `updated_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    -- Constraints
    CONSTRAINT `uk_seasons_slug` UNIQUE (`slug`),
    CONSTRAINT `chk_seasons_status` CHECK (`status` IN ('draft', 'published', 'archived')),
    CONSTRAINT `chk_seasons_display_order` CHECK (`display_order` >= 0)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE INDEX `idx_travel_tips_status` ON `travel_tips`(`status`);
CREATE INDEX `idx_travel_tips_display_order` ON `travel_tips`(`display_order`);
CREATE INDEX `idx_visa_rules_status` ON `visa_rules`(`status`);
CREATE INDEX `idx_visa_rules_country_code` ON `visa_rules`(`country_code`);
CREATE INDEX `idx_seasons_status` ON `seasons`(`status`);
CREATE INDEX `idx_seasons_display_order` ON `seasons`(`display_order`);
...
-- Content Pages
INSERT INTO `content_pages` (`slug`, `title`, `description`, `content`, `meta_title`, `meta_description`, `status`)
VALUES
('about', 'About Us', 'Learn about Drishya Travels', '<p>Drishya Travels is a modern travel guide for discovering Nepal\'s mountains, culture, adventure and wild places.</p>', 'About Drishya Travels', 'Learn about Drishya Travels and our mission to help you discover Nepal.', 'published'),
('contact', 'Contact Us', 'Get in touch with Drishya Travels', '<p>Contact us via phone, email, or WhatsApp for booking enquiries and travel planning.</p>', 'Contact Drishya Travels', 'Get in touch with Drishya Travels for your Nepal travel planning.', 'published'),
('privacy-policy', 'Privacy Policy', 'Our privacy policy', '<p>Your privacy is important to us. This policy explains how we collect, use, and protect your information.</p>', 'Privacy Policy', 'Read our privacy policy for information on how we handle your data.', 'published'),
('terms-and-conditions', 'Terms & Conditions', 'Our terms and conditions', '<p>By using our services, you agree to these terms and conditions.</p>', 'Terms & Conditions', 'Read our terms and conditions for using Drishya Travels services.', 'published')
ON DUPLICATE KEY UPDATE
    `title` = VALUES(`title`),
    `description` = VALUES(`description`),
...
-- Travel Tips (from travel-tips.html)
INSERT INTO `travel_tips` (`slug`, `title`, `icon`, `summary`, `content`, `display_order`, `status`)
VALUES
('pack-for-journey', 'Pack for the Journey', 'backpack', 'Nepal\'s climate varies significantly with altitude and season. Pack according to your destination and activities.', '<ul><li>Comfortable walking shoes are useful for sightseeing and city exploration.</li><li>Bring layers when travelling to mountainous regions.</li><li>Carry rain protection during the monsoon season.</li><li>Sunscreen, sunglasses and a reusable water bottle are useful for outdoor activities.</li></ul>', 0, 'published'),
('money-payments', 'Money & Payments', 'credit-card', 'Nepal uses the Nepalese Rupee (NPR). Cards are accepted at many hotels, restaurants and larger businesses, but cash remains useful, especially outside major cities.', '<ul><li>Carry some Nepalese Rupees for smaller purchases.</li><li>ATMs are more readily available in major cities and tourist areas.</li><li>Keep your foreign exchange receipts when exchanging currency.</li></ul>', 1, 'published'),
('altitude-trekking', 'Altitude & Trekking', 'mountain', 'If your journey includes high-altitude trekking, allow enough time for acclimatization and avoid increasing altitude too quickly.', '<ul><li>Stay hydrated.</li><li>Follow your guide\'s recommendations.</li><li>Take acclimatization days seriously.</li><li>Inform your travel company about relevant medical or fitness concerns before departure.</li></ul>', 2, 'published'),
('respect-local-culture', 'Respect Local Culture', 'heart', 'Nepal has diverse cultures, religions and traditions. Travelling respectfully helps preserve the places and communities you visit.', '<ul><li>Dress appropriately when visiting religious sites.</li><li>Ask before photographing people.</li><li>Remove shoes where local customs require it.</li><li>Respect local religious practices and monuments.</li></ul>', 3, 'published'),
('stay-connected', 'Stay Connected', 'wifi', 'Mobile connectivity is generally available in populated areas, while connectivity can become limited in remote trekking regions.', '<ul><li>Download important maps before heading into remote areas.</li><li>Keep important documents backed up digitally.</li><li>Carry a power bank for longer journeys.</li></ul>', 4, 'published'),
('travel-responsibly', 'Travel Responsibly', 'leaf', 'Help keep Nepal\'s natural and cultural destinations beautiful for future generations.', '<ul><li>Avoid unnecessary single-use plastics.</li><li>Dispose of waste properly.</li><li>Support local businesses and communities.</li><li>Follow rules in national parks and protected areas.</li></ul>', 5, 'published')
ON DUPLICATE KEY UPDATE
    `title` = VALUES(`title`),
    `icon` = VALUES(`icon`),
...
-- Visa Rules (from visa-information.html)
INSERT INTO `visa_rules` (`country_code`, `country_name`, `visa_required`, `visa_on_arrival`, `visa_free_days`, `visa_fee_usd`, `requirements`, `notes`, `official_url`, `status`)
VALUES
('NP', 'Nepal', FALSE, FALSE, NULL, NULL, 'N/A', 'Indian nationals do not require a visa to enter Nepal. Acceptable identity documents include passport, voter ID card, or other government-issued photo ID.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published'),
('IN', 'India', FALSE, FALSE, NULL, NULL, 'Valid passport or government-issued photo ID (voter ID, Aadhar card, etc.)', 'Indian nationals do not require a visa to enter Nepal.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published'),
('US', 'United States', TRUE, TRUE, NULL, 30.00, 'Valid passport (6+ months), passport-size photo, visa fee payment', 'Tourist visa on arrival available at designated entry points. 15 days: $30, 30 days: $50, 90 days: $125.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published'),
('GB', 'United Kingdom', TRUE, TRUE, NULL, 30.00, 'Valid passport (6+ months), passport-size photo, visa fee payment', 'Tourist visa on arrival available at designated entry points. 15 days: $30, 30 days: $50, 90 days: $125.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published'),
('AU', 'Australia', TRUE, TRUE, NULL, 30.00, 'Valid passport (6+ months), passport-size photo, visa fee payment', 'Tourist visa on arrival available at designated entry points. 15 days: $30, 30 days: $50, 90 days: $125.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published'),
('CA', 'Canada', TRUE, TRUE, NULL, 30.00, 'Valid passport (6+ months), passport-size photo, visa fee payment', 'Tourist visa on arrival available at designated entry points. 15 days: $30, 30 days: $50, 90 days: $125.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published'),
('CN', 'China', TRUE, FALSE, NULL, NULL, 'Valid passport, visa obtained prior to arrival', 'Chinese nationals require a visa from a Nepalese diplomatic mission before travelling. Not eligible for visa on arrival.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published'),
('XX', 'Other Countries', TRUE, TRUE, NULL, 30.00, 'Valid passport (6+ months), passport-size photo, visa fee payment', 'Most other nationalities eligible for visa on arrival. Check latest requirements with Nepal Department of Immigration.', 'https://ntb.gov.np/plan-your-trip/before-you-come/tourist-visa', 'published')
ON DUPLICATE KEY UPDATE
    `country_name` = VALUES(`country_name`),
    `visa_required` = VALUES(`visa_required`),
    `visa_on_arrival` = VALUES(`visa_on_arrival`),
    `visa_free_days` = VALUES(`visa_free_days`),
    `visa_fee_usd` = VALUES(`visa_fee_usd`),
...
-- Seasons (from best-time-to-visit.html)
INSERT INTO `seasons` (`slug`, `name`, `months`, `description`, `highlights`, `recommended_for`, `display_order`, `status`)
VALUES
('spring', 'Spring', 'March–May', 'Spring brings warmer conditions and blooming rhododendrons in many trekking regions. It is one of the best periods for trekking and mountain adventures.', 'Blooming rhododendrons, warmer weather, clear mountain views', 'Trekking, mountain adventures, photography', 0, 'published'),
('summer', 'Summer (Monsoon)', 'June–August', 'The monsoon brings substantial rainfall to much of Nepal. Some trekking areas can become wet and more difficult, although rain-shadow destinations such as Mustang can offer different conditions.', 'Lush green landscapes, fewer crowds, rain-shadow trekking (Mustang, Dolpo)', 'Rain-shadow trekking, cultural tours in cities, botanical interest', 1, 'published'),
('autumn', 'Autumn', 'September–November', 'Autumn is one of Nepal\'s most popular travel periods. Conditions are generally favorable for trekking, with October and November often offering clear mountain views.', 'Clear mountain views, stable weather, festival season (Dashain, Tihar)', 'Classic Himalayan trekking, mountain photography, cultural festivals', 2, 'published'),
('winter', 'Winter', 'December–February', 'Winter can be cold at higher elevations, but lower elevations and cities can still be enjoyable. This period can also be suitable for sightseeing and selected lower altitude journeys.', 'Crisp clear skies, peaceful trails, wildlife viewing in lowlands', 'Low-altitude sightseeing, cultural tours, wildlife (Chitwan, Bardia), city exploration', 3, 'published')
ON DUPLICATE KEY UPDATE
    `name` = VALUES(`name`),
...
-- SELECT slug, title, status FROM content_pages;

-- Verify travel tips
-- SELECT slug, title, display_order, status FROM travel_tips ORDER BY display_order;

-- Verify visa rules
-- SELECT country_code, country_name, visa_required, visa_on_arrival, visa_fee_usd, status FROM visa_rules;

-- Verify seasons
-- SELECT slug, name, months, display_order, status FROM seasons ORDER BY display_order;